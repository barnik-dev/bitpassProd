...
def add_password(request):
    ...
    if request.method == "POST":
        ...
        # THERE IS A CHANCE THAT USER CAN TRY TO SAVE PASSWORD WITHOUT CREATING ANY CATEGORY FIRST
        if category_name:
            category = user.categories_set.get(category_name=category_name) # QUERY'IN CHILD (CATEGORY) FROM PARENT (USER
            ...
            # USER HASN'T CREATED ANY CATEGORY
...
def edit_account_password(request, pk):
    ...
    # DECRYPTING THE PASSWORD SO THAT WE CAN PASS THE ACTUAL VALUE TO THE EDIT FORM
    decrypted_password = cipher.decrypt_data(eval(account_password.password))
    ...
    if request.method == 'POST':
        ...
        if title or url or username or email or password:
            ...
            # SINCE EMAIL CANNOT BE EMPTY THERE IS NO NEED TO CHECK
            account_password.email = email

            # SINCE PASSWORD CANNOT BE EMPTY THERE IS NO NEED TO CHECK
    ...
    context = {'categories': categories_of_user, 'account_password' :account_password, 'title_acronym': title_acronym, 'decrypted_password': decrypted_password}
